...
//**************赋值*****************
    bign operator=(const char* num){   //重构赋值
        len=strlen(num);
		xsd=0,much_lin=0;
        for(int i=0,g=0,lent=len-1;i<len;i++){
			if(num[lent-i]!='.')    //去小数点
				s[i-g]=num[lent-i]-'0';
			else xsd=i,g=1;
		}
...
//***********高精度加法***************暂无错误
    bign operator+(const bign& b) const{
		...
		int max_len;
		if(xsd<b.xsd){                 //分情况相加
			for(int i=0,cha=b.xsd-xsd;i<len;i++)
				c.s[i+cha]=s[i];
			max_len=max(b.len,i+cha),i=0;
			for(int g=0;g||i<max_len;i++){
				c.s[i]+=b.s[i];
				g=c.s[i]/10;
				c.s[i]%=10;
			}
			...
			for(int i=0,cha=xsd-b.xsd;i<b.len;i++)
				c.s[i+cha]=b.s[i];
		...
		if(c.xsd&&c.s[0]==0){            //处理后置零
			for(int i=0;i<c.xsd&&c.s[i]==0;i++);
			for(int k=i;i<c.len;i++)
				c.s[i-k]=c.s[i];
			c.len-=k,c.xsd-=k;
		}
        return c;
